...
2. Нужно будет открыть настройки сервера и выполнить базовую конфигурацию. Это делается один раз после установки.
    ...
    * Стягиваем базу данных по доступам из таска. Запускаем Open Server.
    ...
    mysql -u root(имя юзера базы) -ppassword(пароль базы, если его нет, то этот параметр вообще не пишем) dbname(имя базы куда вставляем. Если вы до этого базу в Опен сервере не сделали, то сделайте, иначе не сработает) < dump.sql (имя базы, котороую импортируем)
    ```
